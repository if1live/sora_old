...
TEST(KNVariant, construct)
{
	//int
	Variant v1(1);
	EXPECT_EQ(kVariantTypeInt, v1.type());
	EXPECT_EQ(1, (int)v1);
	
	//float
	Variant v2(2.0f);
	EXPECT_EQ(kVariantTypeFloat, v2.type());
	EXPECT_EQ(2.0f, (float)v2);
	
	//bool
	Variant v3(false);
	EXPECT_EQ(kVariantTypeBool, v3.type());
	EXPECT_EQ(false, (bool)v3);

	//pointer
	int a = 1;
	Variant v4(&a);
	EXPECT_EQ(kVariantTypePointer, v4.type());
	EXPECT_EQ(&a, v4.GetAsPointer<int>());

	//enum
	Variant v5(TestEnum1);
	EXPECT_EQ(kVariantTypeInt, v5.type());
	EXPECT_EQ(TestEnum1, v5.GetAsEnum<TestEnum>());
	EXPECT_EQ(false, v5.IsNull());

	//null
	Variant v6;
	EXPECT_EQ(kVariantTypeNull, v6.type());
...
TEST(KNVariant, set)
{
	...
	v1 = 2.0f;
	EXPECT_EQ(kVariantTypeFloat, v1.type());
	EXPECT_EQ(2.0f, (float)v1);
	
	v1 = false;
	EXPECT_EQ(kVariantTypeBool, v1.type());
	...
	int a = 1;
	v1 = &a;
	EXPECT_EQ(kVariantTypePointer, v1.type());
