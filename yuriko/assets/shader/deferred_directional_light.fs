...
void main() {
	...
	/*
	vec4 screenspace = gl_FragCoord;
	vec4 viewspace = u_projectionInv * screenspace;
	//viewspace *= 0.001;	//1.0 / viewspace.w;
	//viewspace.z = depth;
	gl_FragColor = vec4(screenspace);
	//gl_FragColor = vec4(texture2D(s_pos, v_texcoord).xyz, 1.0);
	*/
}
