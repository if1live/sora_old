...
public:
  Cone(float height, float radius) : m_height(height), m_radius(radius) {
    ParametricInterval interval = { glm::ivec2(20, 20), glm::vec2(kTwoPi, 1), glm::vec2(30, 20) };
...
public:
  MobiusStrip(float scale) : m_scale(scale) {
    ParametricInterval interval = { glm::ivec2(40, 20), glm::vec2(kTwoPi, kTwoPi), glm::vec2(40, 15) };
    SetInterval(interval);
  }
  glm::vec3 Evaluate(const glm::vec2& domain) const {
    float u = domain.x;
